...
VIDEO_CODEC = "libx264"  # Video codec for NVIDIA GPU
...
def compress_video(input_file, output_file, bitrate, video_codec=VIDEO_CODEC, framerate=FRAMERATE):
    try:
        (
            ffmpeg
            .input(input_file)
...
def compress_videos_in_directory(input_directory=INPUT_DIRECTORY, video_codec=VIDEO_CODEC, framerate=FRAMERATE):
    ...
    with tqdm.tqdm(total=len(video_files), desc="Compressing Videos", unit="file") as pbar:
        for filename in video_files:
            ...
            # Compress the video
            compress_video(input_file, output_file, bitrate, video_codec, framerate)
